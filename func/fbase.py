...
def ultimas_noticias():
    ...
    for item in noticias.findAll("item")[
        :5
    ]:  # trocando o numero muda quantas noticas passa
...
def playlist(album):
    if "agora" in album:
        browser.open_new_tab(
            "https://open.spotify.com/playlist/16LIDJwa7vtndNmXw7BJZ5?si=XL0vwvC8Q9-9c3PQRnQsxQ"
        )  # so o open abre janela
    elif "flow" in album:
        browser.open_new_tab(
            "https://open.spotify.com/show/3V5LBozjo4vNg2oJoA4Wb2?si=zpTFt8QfTjeGnHeAEdz3Ew"
        )
    elif "rock" in album:
        browser.open_new_tab(
            "https://open.spotify.com/track/1Mf27cnAF1Q6Ko83XTM5d1?si=f1LVtboZS3CeF6LBbQzI7A"
        )
    elif "pop" in album:
        browser.open_new_tab(
            "https://open.spotify.com/track/738SQjONa0q63yhLxvg3m1?si=gzQctUONSh2PU-O3PIFMUg"
        )
    else:
        browser.open_new_tab(
            "https://open.spotify.com/track/4u7EnebtmKWzUH433cf5Qv?si=QABP8IHuQv-Bg9R7qtC8Fg"
        )
...
def publica_mqtt(topic, payload):
    publish.single(
        ...
        port=1883,
        client_id="ana",
        auth={"username": "cafepuroebom", "password": "cafecomcodigoemuitobom"},
    )
